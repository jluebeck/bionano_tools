...
def plot_alignment(ref_cmaps,ref_contigConv,query_cmaps,query_contigCovs,xmapAln,xmapPairAll,xmapID):
    ...
    #PLOT MAPPED SEGMENTS ABOVE
    if args.segments:
        xStart = RStart-offset 
        for keynum in xmapPairSeg:
            currHeight = 1.5
            contig_aln = xmapPairSeg[keynum]
            alnRefStart = float(contig_aln["RefStartPos"])
            alnRefEnd = float(contig_aln["RefEndPos"])
            qryLen = contig_aln["QryLen"]
            shiftedQryStart = float(contig_aln["QryStartPos"]) + xStart
            shiftedQryEnd = float(contig_aln["QryEndPos"]) + xStart
            #direct = contig_aln["Orientation"]:

            #plt.text(normStart - 10000, -0.55-offset, i,size=4) #make it go over and 1/2
            currHeight+=(0.075*int(contig_aln["RefContigID"]))
            ax.plot([shiftedQryStart, shiftedQryEnd], [currHeight, currHeight], color="k",linewidth=2.5)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-g", "--genes", help="plot gene track, default off", action='store_true')
    parser.add_argument("-s", "--segments", help="xmap of locally aligning segs to plot above contig")
    ...
    if args.segments:
        xmapAlnSeg,xmapPairSeg = parse_xmap(args.segments)
